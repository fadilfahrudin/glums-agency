...
const CardImage = ({ id, img, role, title, desc, lenght }: Props) => {
    ...
    return (
        <motion.div style={{width: `calc(100% / ${lenght})`}} onMouseEnter={() => setHoveredId(id)} onMouseLeave={() => setHoveredId(null)}  whileHover={{ flexBasis: '549px', flexShrink: 0 }} transition={{ duration: 1 }} className={`img-item ${hoveredId == id ? 'active' : ''}`} >
